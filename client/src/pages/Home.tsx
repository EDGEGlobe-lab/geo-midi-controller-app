...
import { updateSettingsMixerTrack } from "@/lib/settingsMixer";
...
import { ParkwayCataloguePanel } from "@/components/ParkwayCataloguePanel";
import { RepositoryMetricsPanel } from "@/components/RepositoryMetricsPanel";
import { SettingsWorkspace } from "@/components/SettingsWorkspace";
import { EngineeringCentrePanel } from "@/components/EngineeringCentrePanel";
import { RouteSignalVisual } from "@/components/RouteSignalVisual";
import { getAdjacentStationProgramme, getStationProgramme, parkwayRadioStations } from "@shared/radioStationCatalog";
import { catalogueSourceUrl, PARKWAY_CATALOGUE_PROJECT_KEY, parkwayCatalogue, parkwaySyntheticVocalVariants, projectAssetPlaybackUrl } from "@shared/parkwayCatalogue";
...
import "@/components/RepositoryMetricsPanel.css";
...
export default function Home() {
  ...
  const [stereoInStatus, setStereoInStatus] = useState<"idle" | "ready" | "error">("idle");
  ...
  const [activeView, setActiveView] = useState<ParkwayWorkspace>(() => {
    ...
    return ["Arrangement", "Mixer", "Piano Roll", "Performance", "Studio", "Catalogue", "Repository", "Settings", "Engineering", "Generator", "Radio", "Product", "Devices", "Develop", "Assets", "History", "Feedback", "Review", "Contact"].includes(requestedView ?? "") ? requestedView as ParkwayWorkspace : "Arrangement";
  });
  ...
  const [catalogueTrackId, setCatalogueTrackId] = useState(parkwayCatalogue[0].id);
  const [catalogueActive, setCatalogueActive] = useState(false);
  const [cataloguePlaybackMode, setCataloguePlaybackMode] = useState<"instrumental" | "synthetic-vocal">("instrumental");
  ...
  const projectKey = PARKWAY_CATALOGUE_PROJECT_KEY;
  ...
  const registerParkwayCatalogue = trpc.studio.assets.registerParkwayCatalogue.useMutation({ onSuccess: (result) => { void assetsQuery.refetch(); toast.success(result.created.length ? `${result.created.length} original masters indexed in your private project assets` : "All PARKWAY catalogue masters are already indexed"); }, onError: (error) => toast.error(error.message) });
  const registerParkwaySyntheticVocals = trpc.studio.assets.registerParkwaySyntheticVocals.useMutation({ onSuccess: (result) => { void assetsQuery.refetch(); toast.success(result.created.length ? `${result.created.length} verified through-composed synthetic vocal masters indexed separately` : "All available verified through-composed vocal masters are already indexed"); }, onError: (error) => toast.error(error.message) });
  ...
  const repositoryMetricsQuery = trpc.repositoryMetrics.live.useQuery(undefined, { enabled: dataPlan.repositoryMetrics, retry: 1, staleTime: 60_000 });
  ...
  const activeCatalogueTrack = parkwayCatalogue.find((track) => track.id === catalogueTrackId) ?? parkwayCatalogue[0];
  const activeSyntheticVocal = parkwaySyntheticVocalVariants.find((variant) => variant.trackId === activeCatalogueTrack.id);
  const isSyntheticVocalSelected = catalogueActive && cataloguePlaybackMode === "synthetic-vocal" && Boolean(activeSyntheticVocal);
  ...
  const previewSource = radioActive ? (selectedRadioProgramme?.sourceUrl ?? selectedRadioStation.sourceUrl) : historySourceUrl ?? fallbackSourceUrl ?? (catalogueActive ? (isSyntheticVocalSelected && activeSyntheticVocal ? projectAssetPlaybackUrl(activeSyntheticVocal.storageKey) : catalogueSourceUrl(activeCatalogueTrack)) : previewAsset ? projectAssetPlaybackUrl(previewAsset.storageKey) : activeTrack.src);
  const previewLabel = radioActive ? `${selectedRadioStation.name} · ${selectedRadioProgramme?.title ?? selectedRadioStation.nowPlaying}` : historySourceLabel ?? (fallbackSelection ? `Night Drive fallback · ${fallbackSelection.genre.label}` : catalogueActive ? `${activeCatalogueTrack.title}${isSyntheticVocalSelected ? " · Synthetic vocal" : ""}` : previewAsset ? previewAsset.filename : activeTrack.label);
  const previewBars = useMemo(() => {
    const bars = catalogueActive ? parseWaveform(JSON.stringify(Array.from({ length: 72 }, (_, index) => 18 + ((activeCatalogueTrack.number * 37 + index * 19 + (index % 7) * 13) % 72)))) : previewAsset ? parseWaveform(previewAsset.waveformPreview) : makeFallbackWaveform(new TextEncoder().encode(activeTrack.label), 64);
  ...
  }, [catalogueActive, activeCatalogueTrack, previewAsset, activeTrack.label, previewLabel, waveformZoom]);
  const previewOptions = useMemo(() => [
    ...AUDIO_TRACKS.map((track) => ({ value: `track:${track.id}`, label: track.label, detail: track.tag })),
    ...parkwayCatalogue.map((track) => ({ value: `catalogue:${track.id}`, label: `${track.number.toString().padStart(2, "0")} · ${track.title}`, detail: `${track.genre.toUpperCase()} · ${track.tempo} BPM · ${track.key}` })),
  ...
  function routeSourceToTrack(trackId: string) {
    const stereoIn = stereoInRef.current;
  ...
  useEffect(() => {
    const profile = BASS_PROFILES[bassProfile];
    window.localStorage.setItem("parkway-bass-profile", bassProfile);
    const context = audioContextRef.current;
    if (!context) return;
    bassShelfRef.current?.frequency.setTargetAtTime(profile.frequencyHz, context.currentTime, 0.03);
    bassShelfRef.current?.gain.setTargetAtTime(profile.bassDb, context.currentTime, 0.03);
    previewGainRef.current?.gain.setTargetAtTime(profile.preamp, context.currentTime, 0.03);
  }, [bassProfile]);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      setStereoMeter(browserStereoMeter(timeData));
  ...
  const startMediaFromGesture = async (successMessage?: string) => {
    ...
    try {
      ...
      if (!await startWithinUserGesture(audio, enableStereo, () => {
        const context = ensureAudioGraph();
        return Boolean(context && routeSourceToTrack(selectedTrack));
      })) return false;
      setIsPlaying(true);
      setStereoStatus("ready");
  ...
  const testBuiltInSpeakerOutput = async () => {
    const context = ensureAudioGraph();
    const stereoIn = stereoInRef.current;
    if (!context || !stereoIn) { setStereoStatus("error"); setStereoInStatus("error"); toast.error("Stereo test output is unavailable in this browser."); return; }
    try {
      if (context.state !== "running") await context.resume();
      setMaster((current) => Math.max(50, current));
      ...
      oscillator.connect(gain).connect(stereoIn);
      setSignalTestActive(true);
      oscillator.start();
      oscillator.stop(context.currentTime + 0.76);
      window.setTimeout(() => setSignalTestActive(false), 900);
      setStereoInStatus("ready"); setChannelStatus("ready"); setMixBusStatus("ready"); setStereoStatus("ready");
      toast.success("Stereo In output test sent. The meter should move during the short tone.");
    } catch (error) {
  ...
  const selectPreviewSource = (value: string) => {
    ...
    if (kind === "catalogue") {
      const nextTrack = parkwayCatalogue.find((track) => track.id === rawId);
      if (!nextTrack) return;
      setPreviewAssetId(null);
      setCatalogueTrackId(nextTrack.id);
      setCatalogueActive(true);
      setCataloguePlaybackMode("instrumental");
      setTempo(nextTrack.tempo);
      return;
    }
    setPreviewAssetId(null);
    setCatalogueActive(false);
    setCurrentTrackId(rawId as (typeof AUDIO_TRACKS)[number]["id"]);
  };
  const selectCatalogueTrack = (trackId: string) => {
    selectPreviewSource(`catalogue:${trackId}`);
    stop();
  };
  const playCatalogueTrack = (trackId: string) => {
    if (catalogueActive && catalogueTrackId === trackId) { void togglePlay(); return; }
    selectCatalogueTrack(trackId);
    window.setTimeout(() => void togglePlay(), 0);
  };
  const selectCataloguePlaybackMode = (mode: "instrumental" | "synthetic-vocal") => {
    const variant = parkwaySyntheticVocalVariants.find((item) => item.trackId === catalogueTrackId);
    if (mode === "synthetic-vocal" && !variant) return;
    stop();
    setCatalogueActive(true);
    setCataloguePlaybackMode(mode);
  };
    ...
    <main className={`parkway-app ${compactMode ? "compact-mode" : ""}`}>
      ...
      <aside id="parkway-workspace-navigation" className={`sidebar ${showBrowser ? "sidebar-open" : "sidebar-collapsed"}`}>
        ...
        {showBrowser && <>
          ...
          <nav className="side-nav">
            {[{ label: "Arrangement", icon: Layers3 }, { label: "Mixer", icon: SlidersHorizontal }, { label: "Piano Roll", icon: Grid3X3 }, { label: "Performance", icon: Zap }, { label: "Studio", icon: Sparkles }, { label: "Catalogue", icon: Disc3 }, { label: "Repository", icon: Code2 }, { label: "Settings", icon: Settings2 }, { label: "Engineering", icon: Boxes }, { label: "Generator", icon: Waves }, { label: "Radio", icon: Radio }, { label: "Product", icon: Gauge }, { label: "Devices", icon: HardDrive }, { label: "Develop", icon: Code2 }, { label: "Assets", icon: FolderOpen }, { label: "History", icon: RotateCcw }, { label: "Feedback", icon: AlertTriangle }, ...(user?.role === "admin" ? [{ label: "Review", icon: ShieldCheck }] : []), { label: "Contact", icon: Radio }].map(({ label, icon: Icon }) => <button key={label} className={`side-link ${activeView === label ? "is-active" : ""}`} onClick={() => { setActiveView(label as ParkwayWorkspace); if (window.innerWidth <= 760) setShowBrowser(false); }}><Icon size={15} /><span>{label}</span>{label === "Performance" && <span className="live-dot" />}</button>)}
          </nav>
      ...
      <section className="workspace">
        ...
        <div className="content-scroll"><div className="workspace-heading"><div><div className="section-kicker"><Radio size={13} /> {activeView} / MASTER SESSION</div><h1>Master bus.<br /><em>Ready to move.</em></h1><p className="heading-copy">Transport, timing, and signal routing in one tactile performance surface.</p><div className="master-readout"><div className="readout-scope"><span /><span /><span /><span /><span /><span /><span /><span /><span /><span /><span /><span /></div><div className="readout-meta"><span>MASTER / STEREO</span><strong>{master}%</strong><small>{formatMeterDb(stereoMeter.peakDb)} peak · {formatMeterDb(stereoMeter.headroomDb)} headroom</small></div><div className="readout-state"><span className="status-light" /> {stereoMeter.peakDb === null ? "METER ARMED" : "SIGNAL ACTIVE"}</div></div></div><div className="heading-tools"><button className="outline-button" onClick={() => toast("New track added to the session")}><Plus size={14} /> Add track</button><button className="solid-button" onClick={() => toast("Render queue started")}><Zap size={14} /> Render</button></div></div>
          ...
          {activeView === "Catalogue" && <><div className="panel-header-actions catalogue-index-action"><span className="small-pill">ORIGINAL · 20 MASTERS</span><button className="outline-button outline-small" disabled={registerParkwayCatalogue.isPending || registerParkwaySyntheticVocals.isPending} onClick={() => { if (!isAuthenticated) { startLogin(); return; } void registerParkwayCatalogue.mutateAsync({ projectKey }); }}>{registerParkwayCatalogue.isPending ? "Indexing…" : isAuthenticated ? "Index private masters" : "Sign in to index"}</button><button className="outline-button outline-small" disabled={registerParkwaySyntheticVocals.isPending} onClick={() => { if (!isAuthenticated) { startLogin(); return; } void registerParkwaySyntheticVocals.mutateAsync({ projectKey }); }}>{registerParkwaySyntheticVocals.isPending ? "Indexing vocals…" : "Index available vocal masters"}</button></div><ParkwayCataloguePanel tracks={parkwayCatalogue} selectedTrackId={catalogueTrackId} currentTime={catalogueActive ? currentTime : 0} duration={catalogueActive ? duration : activeCatalogueTrack.durationMs / 1000} isPlaying={catalogueActive && isPlaying} vocalVariantSelected={isSyntheticVocalSelected} vocalVariantAvailable={Boolean(activeSyntheticVocal)} onSelect={selectCatalogueTrack} onSelectPlaybackMode={selectCataloguePlaybackMode} onPlay={playCatalogueTrack} onTogglePlay={() => void togglePlay()} onScrub={scrubPreview} /></>}
          {activeView === "Repository" && <RepositoryMetricsPanel result={repositoryMetricsQuery.data} isLoading={repositoryMetricsQuery.isLoading} isRefreshing={repositoryMetricsQuery.isFetching && !repositoryMetricsQuery.isLoading} error={repositoryMetricsQuery.error?.message} onRefresh={() => void repositoryMetricsQuery.refetch()} />}
          {activeView === "Settings" && <SettingsWorkspace master={master} bassProfile={bassProfile} selectedTrackId={selectedTrack} tracks={tracksState} compactMode={compactMode} stereoStatus={stereoStatus} stereoInStatus={stereoInStatus} channelStatus={channelStatus} mixBusStatus={mixBusStatus} onMasterChange={(value) => setMaster(clampMasterVolume(value))} onBassProfileChange={setBassProfile} onSelectedTrackChange={setSelectedTrack} onTrackChange={(id, update) => setTracksState((items) => updateSettingsMixerTrack(items, id, update))} onCompactChange={(next) => { setCompactMode(next); setShowBrowser(!next); }} />}
          {activeView === "Engineering" && <EngineeringCentrePanel completedVocalTracks={parkwaySyntheticVocalVariants.length} totalCatalogueTracks={parkwayCatalogue.length} onOpenCatalogue={() => setActiveView("Catalogue")} />}
