...
import { catalogueSourceUrl, PARKWAY_CATALOGUE_PROJECT_KEY, parkwayCatalogue, parkwaySyntheticVocalVariants } from "@shared/parkwayCatalogue";
...
export default function Home() {
  ...
  const [activeView, setActiveView] = useState<ParkwayWorkspace>(() => {
    ...
    return ["Arrangement", "Mixer", "Piano Roll", "Performance", "Studio", "Catalogue", "Generator", "Radio", "Product", "Devices", "Develop", "Assets", "History", "Feedback", "Review", "Contact"].includes(requestedView ?? "") ? requestedView as ParkwayWorkspace : "Arrangement";
  });
  ...
  const previewSource = radioActive ? (selectedRadioProgramme?.sourceUrl ?? selectedRadioStation.sourceUrl) : historySourceUrl ?? fallbackSourceUrl ?? (catalogueActive ? (isSyntheticVocalSelected ? `/manus-storage/${activeSyntheticVocal?.storageKey}` : catalogueSourceUrl(activeCatalogueTrack)) : previewAsset ? `/manus-storage/${previewAsset.storageKey}` : activeTrack.src);
